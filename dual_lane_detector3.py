...
[수정 내용]
- Team2(서버 후처리 결과) 표시 로직을 "폴더에 계속 쌓이는 최신 jpg/json" 구조로 변경
  - 이미지: ~/received_team2_images/{danger,error,safe}/*.jpg
    -> 최신 jpg 2개를 (Camera 0, Camera 1)로 표시 (없으면 1개를 양쪽에 복제)
  - json:   ~/received_team2_jsons/{danger,error,safe}/*.json
    -> 파일명 매칭이 불가능하므로 해당 클래스 폴더의 최신 json 1개 로드
  - 시간 표시는:
    -> img_1767867130199.jpg (epoch ms) 우선 파싱
    -> 아니면 json 파일명 *_YYYY_MM_DD_HH_MM_SS_* 파싱
"""
...
class DualCameraLaneDetector(Node):
    def __init__(self):
        ...
        self.team2_image_0 = None        # Camera 0 슬롯에 띄울 이미지
        self.team2_image_1 = None        # Camera 1 슬롯에 띄울 이미지
        self.team2_json = None           # dict
        self.team2_last_key = None       # (cls, cam0_path, cam1_path, mtime) 형태로 변경
    ...
    def _find_latest_team2_images(self) -> Tuple[Optional[str], Optional[Path], Optional[Path]]:
        ...
        - 폴더 내 최신 jpg 2개를 (cam0, cam1) 슬롯으로 사용
        - 1개만 있으면 둘 다 동일 이미지로 표시
        """
        latest_cls = None
        latest_cam0 = None
        latest_cam1 = None
        ...
        for cls in self.team2_classes:
            ...
            p0 = jpgs[0][1]
            p1 = jpgs[1][1] if len(jpgs) >= 2 else p0

            mt = max(jpgs[0][0], jpgs[1][0] if len(jpgs) >= 2 else jpgs[0][0])
    ...
    def _load_team2_from_disk_if_needed(self):
        """
        최신 팀2 jpg 1~2장을 확인하고 변경되었으면:
          - 이미지 로드
          - 해당 클래스 폴더의 최신 json 1개 로드 (파일명 매칭 불가 -> 최신 기준)
        """
        ...
        try:
            mt0 = cam0_path.stat().st_mtime
            mt1 = cam1_path.stat().st_mtime
        except Exception:
            return

        key = (cls, str(cam0_path), str(cam1_path), max(mt0, mt1))
        ...
        # json: 해당 cls 폴더에서 최신 1개
        data = None
        djson = self.team2_json_root / cls
        latest_json = None
        latest_json_mtime = -1.0

        if djson.exists():
            for p in djson.glob("*.json"):
                try:
                    mt = p.stat().st_mtime
                    if mt > latest_json_mtime:
                        latest_json_mtime = mt
                        latest_json = p
                except Exception:
                    continue
    ...
    def _parse_timestamp_from_filename(self, any_name):
        """
        시간 정보 추출 (jpg/json 파일명 모두 지원)

        지원:
        - img_1767867130199.jpg  (epoch ms)
        - 000_2025_12_11_19_58_45_0.json  (YYYY_MM_DD_HH_MM_SS)
        - 2026.01.08_15_30_45 같은 형태도 유지
        """
        try:
            s = str(any_name)

            # 1) img_1767867130199.jpg (epoch ms)
            m = re.search(r"img_(\d{10,16})", s)
            if m:
                v = int(m.group(1))
                if v >= 10**12:  # ms
                    v = v / 1000.0
            ...
            # 2) *_YYYY_MM_DD_HH_MM_SS_* (json)
            m = re.search(r"(\d{4})_(\d{2})_(\d{2})_(\d{2})_(\d{2})_(\d{2})", s)
            if m:
                y, mo, d, h, mi, se = map(int, m.groups())
                return datetime(y, mo, d, h, mi, se)

            # 3) 2026.01.08_15_30_45 (기존)
            base = Path(s).stem
    ...
    def _draw_team2_panel(self, frame, x1, y1, x2, y2):
        ...
        좌표는 (x1,y1) ~ (x2,y2)
        왼쪽: camera_0 슬롯, 오른쪽: camera_1 슬롯
        상단에 팝업으로 시간 정보 표시
        """
        # 배경
        ...
        with self.team2_lock:
            ...
            data = self.team2_json.copy() if isinstance(self.team2_json, dict) else None  # noqa: F841
        ...
        # 시간 정보 추출 (UPDATED: last_key 구조 변경)
        timestamp_dt = None
        if last_key is not None:
            # last_key = (cls, cam0_path_str, cam1_path_str, mtime)
            try:
                cam0_path_str = last_key[1]
                timestamp_dt = self._parse_timestamp_from_filename(cam0_path_str)
                if timestamp_dt is None:
                    # 혹시 cam1 쪽이 더 최신이면 cam1도 시도
                    cam1_path_str = last_key[2]
                    timestamp_dt = self._parse_timestamp_from_filename(cam1_path_str)
            except Exception:
                timestamp_dt = None
